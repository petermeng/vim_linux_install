...
macro AutoExpand()
{
    ...
    /*获取COMPANY信息*/
    szMyCompany = getreg(MYCOMPANY)
    if (strlen( szMyCompany ) == 0)
    {
        szMyCompany = Ask("Enter your company:")
        setreg(MYCOMPANY, szMyCompany)
    }
    ...
    /*added by mengxiangning for simcom function begin*/
    else if (wordinfo.szWord == "re")/*更改修改原因*/
    {
        DelBufLine(hbuf, ln)
        AddPromblemDescription()
        return
    }
    else if (wordinfo.szWord == "mp")/*更改宏的前缀名*/
    {
        DelBufLine(hbuf, ln)
        AddMacroPrefix()
        return
    }
    else if (wordinfo.szWord == "shi")
    {
        InsertHistory(hbuf,ln+1,language,1)
        DelBufLine(hbuf, ln)
        return
    }
    /*added by mengxiangning for simcom function end*/
    /*配置命令执行*/
    ...
    {
        InsertHistory(hbuf,ln+1,language,0)
        DelBufLine(hbuf, ln)
        return
    }
    ...
    {
        ExpandProcCN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel, szMyCompany)
    }
}
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    {
        DelBufLine(hbuf, ln)
        CreateFunctionDef(hbuf,szMyName,1,0,1)
        return
    }
    else if (szCmd == "hdn")
    {
...
*****************************************************************************/
macro ExpandProcCN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel,szMyCompany)
{
    szCmd = wordinfo.szWord
    hwnd = GetCurrentWnd()
    if (hwnd == 0)
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    else if (szCmd == "hd")
    {
        DelBufLine(hbuf, ln)
        /*生成C语言的头文件*/
        CreateFunctionDef(hbuf,szMyName,0,0,1)
        return
    }
    else if (szCmd == "hdn")
    {
    ...
    else if (szCmd == "shd")
    {
        DelBufLine(hbuf, ln)
        /*生成C语言的头文件*/
        CreateFunctionDef(hbuf,szMyName,0,1,1)
        return
    }
    else if (szCmd == "nhd")
    {
        DelBufLine(hbuf, ln)
        /*生成C语言的头文件*/
        CreateFunctionDef(hbuf,szMyName,0,1,0)
        return
    }
    else if (szCmd == "cr")
    {
        DelBufLine(hbuf, ln)
        numLine = 0
        crType = Ask("请输入问题类型(0:PR,1:CR,2:修改多个bug):")
        while(strlen(crType) != 1)
        {
          crType = Ask("请输入问题类型(0:PR,1:CR,2:修改多个bug):")
        }
        if (crType == 0)
        ...
        if (crType == 2)
        {
            crTypeTemp = Ask("请输入问题类型(0:PR,1:CR,2:退出")
            if (crTypeTemp != 2)
                crTestTemp = Ask("请输入哪个部门测试(HT/ST/SH/NO):")
            if (crTypeTemp == 0)
            {
                prNumberTemp = Ask("请输入PR号:")
                crReason = cat("[PR_",prNumberTemp)
                crReason = cat(crReason, "][")
                crReason = cat(crReason, crTestTemp)
                crReason = cat(crReason, "]")
                crReasonTemp = Ask("请输入修改点:")
                crReason = cat(crReason, crReasonTemp)
                InsBufLine(hbuf, ln + numLine, "@crReason@")
                numLine = numLine + 1
            }
            else if (crTypeTemp == 1)
            {
                crReason = cat("[CR][",crTestTemp)
                crReason = cat(crReason, "]")
                crReasonTemp = Ask("请输入修改点:")
                crReason = cat(crReason, crReasonTemp)
                InsBufLine(hbuf, ln + numLine, "@crReason@")
        ...
        if (crType != 2)
            crTest = Ask("请输入哪个部门测试(HT/ST/SH/NO):")
        crCause = Ask("请输入Cause:")
        crAction = Ask("请输入Action:")
        crTestFlow = Ask("请输入TestFlow:")
        crTestResult = Ask("请输入TestResult:")
        crRelated_Module = Ask("请输入Related_Module:")
        
        if ((strlen(crReason) > 0) && (strlen(crTest) > 0))
        {
            if (crType == 0)
            {
                InsBufLine(hbuf, ln + numLine, "[PR_@prNumber@][@crTest@]@crReason@")
                numLine = numLine + 1
            }
            else if (crType == 1)
            {
                InsBufLine(hbuf, ln + numLine, "[CR][@crTest@]@crReason@")
                numLine = numLine + 1
            }
        }
        
        if (strlen(crReason) > 0)
        {
            if (crType != 2)
            {
                InsBufLine(hbuf, ln + numLine, "@crReason@")
        ...
        if (strlen(crCause) > 0)
        {
            InsBufLine(hbuf, ln + numLine, "Cause: @crCause@")
        }
        else
        {
            InsBufLine(hbuf, ln + numLine, "Cause:")
        }
        numLine = numLine + 1

        if (strlen(crAction) > 0)
        {
            InsBufLine(hbuf, ln + numLine, "Action: @crAction@")
        }
        else
        {
            InsBufLine(hbuf, ln + numLine, "Action:")
        }
        numLine = numLine + 1

        if (strlen(crTestFlow) > 0)
        {
            InsBufLine(hbuf, ln + numLine, "TestFlow: @crTestFlow@")
        }
        else
        {
            InsBufLine(hbuf, ln + numLine, "TestFlow:")
        }
        numLine = numLine + 1

        if (strlen(crTestResult) > 0)
        {
            InsBufLine(hbuf, ln + numLine, "TestResult: @crTestResult@")
        }
        else
        {
            InsBufLine(hbuf, ln + numLine, "TestResult:")
        }
        numLine = numLine + 1

        if (strlen(crRelated_Module) > 0)
        {
            InsBufLine(hbuf, ln + numLine, "Related_Module: @crRelated_Module@")
        }
        else
        {
            InsBufLine(hbuf, ln + numLine, "Related_Module:")
        }
    }
    else if (szCmd == "cc")
    {
        ...
        crTest = Ask("请输入哪个部门测试(HT/ST/SH/NO):")
        crTopic = Ask("请输入Topic")
        if (crType == 2)
        {
            InsBufLine(hbuf, ln + numLine, "Title: [PR][@crTest@]修改多个bug")
            numLine = numLine + 1
            crTypeTemp = Ask("请输入问题类型(0:PR,1:CR,2:退出")
            if (crTypeTemp != 2)
                crTestTemp = Ask("请输入哪个部门测试(HT/ST/SH/NO):")
    ...
    else if (szCmd == "mab")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ add for bug[@szQuestion@] \@@sz@@szMonth@@szDay@ begin*/");
        }
        else
        {
            szQuestionDescription = GetReg ("PD")
            if (strlen(szQuestionDescription) > 0)
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ add for @szQuestionDescription@ \@@sz@@szMonth@@szDay@ begin*/");        
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ add for 原因 \@@sz@@szMonth@@szDay@ begin*/");        
            }
        }
        return
    }
    else if (szCmd == "mae")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ add for bug[@szQuestion@] \@@sz@@szMonth@@szDay@ end*/");
        }
        else
        {
            szQuestionDescription = GetReg ("PD")
            if (strlen(szQuestionDescription) > 0)
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ add for @szQuestionDescription@ \@@sz@@szMonth@@szDay@ end*/");        
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ add for 原因 \@@sz@@szMonth@@szDay@ end*/");        
            }
        }
        return
    }
    else if (szCmd == "mdb")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ delete for bug[@szQuestion@] \@@sz@@szMonth@@szDay@ begin*/");
        }
        else
        {
            szQuestionDescription = GetReg ("PD")
            if (strlen(szQuestionDescription) > 0)
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ delete for @szQuestionDescription@ \@@sz@@szMonth@@szDay@ begin*/");        
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ delete for 原因 \@@sz@@szMonth@@szDay@ begin*/");        
            }
        }
    ...
    else if (szCmd == "mde")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ delete for bug[@szQuestion@] \@@sz@@szMonth@@szDay@ end*/");
        }
        else
        {
            szQuestionDescription = GetReg ("PD")
            if (strlen(szQuestionDescription) > 0)
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ delete for @szQuestionDescription@ \@@sz@@szMonth@@szDay@ end*/");        
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ delete for 原因 \@@sz@@szMonth@@szDay@ end*/");        
            }
        }
        return
    }
    else if (szCmd == "mmb")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ modify for bug[@szQuestion@] \@@sz@@szMonth@@szDay@ begin*/");
        }
        else
        {
            szQuestionDescription = GetReg ("PD")
            if (strlen(szQuestionDescription) > 0)
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ modify for @szQuestionDescription@ \@@sz@@szMonth@@szDay@ begin*/");        
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ modify for 原因 \@@sz@@szMonth@@szDay@ begin*/");        
            }
        }
        return
    }
    else if (szCmd == "mme")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ modify for bug[@szQuestion@] \@@sz@@szMonth@@szDay@ end*/");
        }
        else
        {
            szQuestionDescription = GetReg ("PD")
            if (strlen(szQuestionDescription) > 0)
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ modify for @szQuestionDescription@ \@@sz@@szMonth@@szDay@ end*/");        
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ modify for 原因 \@@sz@@szMonth@@szDay@ end*/");        
            }
        }
        return
    }
    else if (szCmd == "mas")
    {
       	...
       	if (sz3 < 10)
        {
       	...
       	else
       	{
 		    ...
 		    szLine1 = strmid (GetBufLine(hbuf, ln), 0, GetBufLineLength (hbuf, ln)-3)
        DelBufLine(hbuf, ln)
        
        szQuestion = GetReg ("PNO")
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ add for bug[@szQuestion@] \@@sz@@szMonth@@szDay@*/");
        }
        else
        {
            szQuestionDescription = GetReg ("PD")
            if (strlen(szQuestionDescription) > 0)
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ add for @szQuestionDescription@ \@@sz@@szMonth@@szDay@*/");        
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ add for 原因 \@@sz@@szMonth@@szDay@*/");        
            }
        }
        return
    }
    else if (szCmd == "mds")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ delete for bug[@szQuestion@] \@@sz@@szMonth@@szDay@*/");
        }
        else
        {
            szQuestionDescription = GetReg ("PD")
            if (strlen(szQuestionDescription) > 0)
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ delete for @szQuestionDescription@ \@@sz@@szMonth@@szDay@*/");        
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ delete for 原因 \@@sz@@szMonth@@szDay@*/");        
            }
        }
        return
    }
    else if (szCmd == "mms")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ modify for bug[@szQuestion@] \@@sz@@szMonth@@szDay@*/");
        }
        else
        {
            szQuestionDescription = GetReg ("PD")
            if (strlen(szQuestionDescription) > 0)
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ modify for @szQuestionDescription@ \@@sz@@szMonth@@szDay@*/");        
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* @szMyCompany@ @szMyName@ modify for 原因 \@@sz@@szMonth@@szDay@*/");        
            }
        }
        return
    }
    else if (szCmd == "sfile" || szCmd == "sfi" )
    {
        DelBufLine(hbuf, ln)
        /*生成文件头说明*/
        InsertSimFileHeaderCN( hbuf,0, szMyName,"",0)
        return
    }
    else if (szCmd == "senum" || szCmd == "sen")
    {
        DelBufLine(hbuf, ln)
        //提示输入枚举名并转换为大写
...
*****************************************************************************/
macro InsertSimFileHeaderCN(hbuf, ln,szName,szContent,isHeadFile)
{
    hnewbuf = newbuf("")
    if(hnewbuf == hNil)
    {
    ...
    InsBufLine(hbuf, ln + 4,  "*                Copyright (c) @szYear@ @szMyCompany@")
    InsBufLine(hbuf, ln + 5,  "*")
    InsBufLine(hbuf, ln + 6,  "*****************************************************************************")
    szModuleName = Ask("请输入模块名：")
    InsBufLine(hbuf, ln + 7,  "*  模块名	： @szModuleName@")
    sz = GetFileName(GetBufName (hbuf))
    InsBufLine(hbuf, ln + 8,  "*  文件名	： @sz@")
    iLen = strlen (szContent)
    nlnDesc = ln
    szTmp = "*  实现功能："
    InsBufLine(hbuf, ln + 9,   "*  实现功能：@szContent@")
    InsBufLine(hbuf, ln + 10,  "*  作者		： @szName@")
    InsBufLine(hbuf, ln + 11,  "*****************************************************************************")
    InsBufLine(hbuf, ln + 12,  "*  修改记录:")
    InsBufLine(hbuf, ln + 13,  "*")
    InsBufLine(hbuf, ln + 14,  "*  日  期                 修改人	                修改内容")
    InsBufLine(hbuf, ln + 15,  "*  @szYear@/@szMonth@/@szDay@         @szName@        Create")
    InsBufLine(hbuf, ln + 16,  "*****************************************************************************/")

    if (isHeadFile == 0)
    {
        szMacroCtl = toupper(Ask("请输入宏名字:"))
        if (szMacroCtl == "#")
        {
            szMacroCtl = ""
        }

        if (strlen(szMacroCtl) > 0)
        {
            szMacroPrefix = getreg("MP")
            if (strlen(szMacroPrefix) == 0)
            {
                InsBufLine(hbuf, ln + 17, "#if defined (@szMacroCtl@)")
            }
            else
            {
                InsBufLine(hbuf, ln + 17, "#if defined (__@szMacroPrefix@_@szMacroCtl@__)")
            }
        }
        else
        {
        ...
        InsBufLine(hbuf, ln + 18, "")
        InsBufLine(hbuf, ln + 19, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 20, "* Include Files")
        InsBufLine(hbuf, ln + 21, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 22, "")
        InsBufLine(hbuf, ln + 23, "")

        InsBufLine(hbuf, ln + 24, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 25, "* Macros")
        InsBufLine(hbuf, ln + 26, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 27, "")
        InsBufLine(hbuf, ln + 28, "")

        InsBufLine(hbuf, ln + 29, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 30, "* Macro-functions")
        InsBufLine(hbuf, ln + 31, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 32, "")
        InsBufLine(hbuf, ln + 33, "")

        InsBufLine(hbuf, ln + 34, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 35, "* Types")
        InsBufLine(hbuf, ln + 36, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 37, "")
        InsBufLine(hbuf, ln + 38, "")

        InsBufLine(hbuf, ln + 39, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 40, "* Extern Variables (Extern 、Global)")
        InsBufLine(hbuf, ln + 41, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 42, "")
        InsBufLine(hbuf, ln + 43, "")

        InsBufLine(hbuf, ln + 44, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 45, "* External Functions Prototypes")
        InsBufLine(hbuf, ln + 46, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 47, "")
        InsBufLine(hbuf, ln + 48, "")

        InsBufLine(hbuf, ln + 49, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 50, "* Local Variables (static ,Global)")
        InsBufLine(hbuf, ln + 51, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 52, "")
        InsBufLine(hbuf, ln + 53, "")

        InsBufLine(hbuf, ln + 54, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 55, "* Local Functions Prototypes")
        InsBufLine(hbuf, ln + 56, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 57, "")
        InsBufLine(hbuf, ln + 58, "")

        InsBufLine(hbuf, ln + 59, "")
        if (strlen(szMacroCtl) > 0)
        {
            InsBufLine(hbuf, ln + 60, "#endif /* __@szMacroPrefix@_@szMacroCtl@__ */")
        }
    }
    ...
    SetBufIns(hbuf,iTotalLn,0)
    //SetBufIns(hbuf,nlnDesc + 60,0)
    
    //将原"文件描述"行删掉
    DelBufLine(hbuf,nlnDesc + 9)
    
    //自动排列显示功能描述
    CommentContent(hbuf,nlnDesc + 9,"*  实现功能：",szContent,0)
}
/*added by mengxiangning for simcom function end*/
...
//    InsBufLine(hbuf, ln+9, " *   修改内容: 新生成函数")    
    ...
    {
        isFirstInputParam = 1
        isFirstOutputParam = 1
            
        //提示输入新函数的返回值
        szRet = Ask("请输入返回值类型")
        if(strlen(szRet) > 0)
        {
            szReturnDescribe = ask("请输入返回值的描述")
            PutBufLine(hbuf, ln+6, " * 返 回 值  : @szRet@:  @szReturnDescribe@")            
        ...
        while (1)
        {
            while(1)
            {
                isReturnParam = Ask("是否为输出参数(0:N/1:Y)?")
                if ((isReturnParam == 1) || (isReturnParam == 0))
                {
                    break;
                }
            }
            ...
            {
                if (isFirstInputParam == 1)
                {
                    szIns = " * 输入参数  : "
                }
            }
        
            szParamType = ask("请输入函数参数类型")
            szParamType = TrimString(szParamType)
            szParamType = cat(szParamType, " ")
        
            szParam = ask("请输入函数参数名")
            szParam = TrimString(szParam)
            szParamOriginal = szParam
            szTmp = cat(szIns,szParam)
            szParam = cat(szParamType, szParam)
            szParam = cat(szFuncDef,szParam)
            
            //光标定位到函数的最后一个参数后
            sel.lnFirst = ln + 11 + countReturnParam + countInputParam
            sel.lnLast = ln + 11 + countReturnParam + countInputParam
            setWndSel(hwnd,sel)
            ...
            if (isReturnParam == 1)
            {
                if(isFirstOutputParam == 1)
                {
                    PutBufLine(hbuf, ln+5+countReturnParam+countInputParam, "@szTmp@")
                    isFirstOutputParam  = 0
                }
                else
                {
                    countReturnParam = countReturnParam + 1
                    InsBufLine(hbuf, ln+5+countReturnParam+countInputParam, "@szTmp@")
                    oldsel.lnFirst = ln + 11 + countReturnParam + countInputParam
                    oldsel.lnLast = ln + 11 + countReturnParam + countInputParam
                }
            }
            else
            {
                if(isFirstInputParam == 1)
                {
                    PutBufLine(hbuf, ln+4+countInputParam, "@szTmp@")
                    isFirstInputParam  = 0
                }
                else
                {
                    countInputParam = countInputParam + 1
                    InsBufLine(hbuf, ln+4+countInputParam, "@szTmp@")
                    oldsel.lnFirst = ln + 11 + countReturnParam + countInputParam
                    oldsel.lnLast = ln + 11 + countReturnParam + countInputParam
                }
            }
            ...
            //光标定位到函数"{"后的"#"处，并选中"#"
            oldsel.lnFirst = ln + 13 + countReturnParam + countInputParam
            oldsel.lnLast = ln + 13 + countReturnParam + countInputParam

            oldsel.ichFirst = 4
            oldsel.ichLim = 5
            setWndSel(hwnd,oldsel)

            szExit = toupper(Ask("是否继续输入参数(y/n)?"))
            if (szExit == "N")
            {
                break
            }
    ...
    return ln + 14 + countReturnParam + countInputParam
}
...
*****************************************************************************/
macro InsertHistory(hbuf,ln,language,isSim)
{
    iHistoryCount = 1
//    isLastLine = ln
//    i = 0
...
//    }
    if(language == 0)
    {
        if (isSim == 1)
        {
            InsertSimHistoryContentCN(hbuf,ln,iHistoryCount)
        }
        else
        {
...
/*****************************************************************************
 函 数 名  : InsertSimHistoryContentCN
 功能描述  : 插入历史修改记录中文说明
 输入参数  : hbuf           
             ln             
             iHostoryCount  
...
*****************************************************************************/
macro  InsertSimHistoryContentCN(hbuf,ln,iHostoryCount)
{
	//获取时间
    SysTime = GetSysTime(1);
    szYear=SysTime.Year
	...
	//加上修改日期，修改人
    szTmp = "*  @szYear@/@szMonth@/@szDay@         @szMyName@        "
...
*****************************************************************************/
macro CreateFunctionDef(hbuf, szName, language, isSim, isCpp)
{
    ln = 0
    ...
    InsertCPP(hOutbuf,0,isSim, isCpp)
    HeadIfdefStr(szPreH, isSim)
    ...
    if(language == 0)
    {
        ...
        if (isSim)
        {
            InsertSimFileHeaderCN(hOutbuf,0,szName,szContent,1)
        }
        else
        {
...
macro CreateNewHeaderFile()
{
    ...
    SetCurrentBuf(hOutbuf)
    InsertCPP(hOutbuf,0,0,1)
    HeadIfdefStr(szPreH,0)
    szContent = GetFileName(GetBufName (hbuf))
    if(language == 0)
    {
        szContent = cat(szContent," 的头文件")
...
*****************************************************************************/
macro AddPromblemDescription()
{
    szQuestionDescription = ASK("Please Input problem description ");
    if(szQuestionDescription == "#")
    {
       szQuestionDescription = ""
       SetReg ("PD", "")
    }
    else
    {
       SetReg ("PD", szQuestionDescription)
    }
    return szQuestionDescription
}

/*****************************************************************************
 函 数 名  : AddMacroPrefix
 功能描述  : 添加宏前缀
 输出参数  : 无
 返 回 值  : 
 调用函数  : 
...
*****************************************************************************/
macro AddMacroPrefix()
{
    szMacroPrefix = toupper(ASK("Please Input Macro Prefix "))
    if(szMacroPrefix == "#")
    {
       szMacroPrefix = ""
       SetReg ("MP", "")
    ...
    {
       SetReg ("MP", szMacroPrefix)
    }
    return szMacroPrefix
}
/*added by mengxiangning for simcom function end*/
/*
...
*****************************************************************************/
macro InsertCPP(hbuf,ln,isSim,isCpp)
{
    if (isCpp)
    {
        InsBufLine(hbuf, ln, "")
    ...
    if (isSim == 1)
    {
        ...
        InsBufLine(hbuf, ln + 8, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 9, "* Include Files")
        InsBufLine(hbuf, ln + 10, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 11, "")
        InsBufLine(hbuf, ln + 12, "")

        InsBufLine(hbuf, ln + 13, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 14, "* Macros")
        InsBufLine(hbuf, ln + 15, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 16, "")
        InsBufLine(hbuf, ln + 17, "")

        InsBufLine(hbuf, ln + 18, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 19, "* Macro-functions")
        InsBufLine(hbuf, ln + 20, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 21, "")
        InsBufLine(hbuf, ln + 22, "")

        InsBufLine(hbuf, ln + 23, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 24, "* Types")
        InsBufLine(hbuf, ln + 25, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 26, "")
        InsBufLine(hbuf, ln + 27, "")

        InsBufLine(hbuf, ln + 28, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 29, "* Extern Variables (Extern 、Global)")
        InsBufLine(hbuf, ln + 30, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 31, "")
        InsBufLine(hbuf, ln + 32, "")

        InsBufLine(hbuf, ln + 33, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 34, "* External Functions Prototypes")
        InsBufLine(hbuf, ln + 35, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 36, "")
        InsBufLine(hbuf, ln + 37, "")

        InsBufLine(hbuf, ln + 38, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 39, "* Local Variables (static ,Global)")
        InsBufLine(hbuf, ln + 40, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 41, "")
        InsBufLine(hbuf, ln + 42, "")

        InsBufLine(hbuf, ln + 43, "/***********************************************************************************")
        InsBufLine(hbuf, ln + 44, "* Local Functions Prototypes")
        InsBufLine(hbuf, ln + 45, "***********************************************************************************/")
        InsBufLine(hbuf, ln + 46, "")
        InsBufLine(hbuf, ln + 47, "")
...
macro HeadIfdefStr(sz,isSim)
{
    ...
    if (isSim == 1)
    {
        ...
        if (strlen(szMacroCtl) > 0)
        {
            ...
            if (strlen(szMacroPrefix) == 0)
            {
                InsBufLine(hbuf, lnFirst, "#if defined (@szMacroCtl@)")
            }
            else
            {
                InsBufLine(hbuf, lnFirst, "#if defined (__@szMacroPrefix@_@szMacroCtl@__)")
            }
        }
        else
        {
    ...
    if (isSim == 1)
    {
        if (strlen(szMacroCtl) > 0)
        {
            InsBufLine(hbuf, iTotalLn, "#endif /* __@szMacroPrefix@_@szMacroCtl@__ */")
        }
    }
...
macro HeaderFileCreate()
{
   ...
   CreateFunctionDef(hbuf,szMyName,language,0,1)
}
